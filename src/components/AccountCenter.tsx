...
import { X, LogOut, Upload, Shield, Zap, RotateCcw, Bell, User, Calendar } from "lucide-react";
...
export default function AccountCenter({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
      <div className="glass-strong rounded-2xl p-5 w-full max-w-sm max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex-1 min-w-0">
            {editingProfile ? (
              <div className="space-y-2">
                <Input value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="bg-secondary border-border h-8 text-sm" placeholder="Display Name" />
                <div className="flex gap-2">
                  <Button size="sm" onClick={handleUpdateProfile} className="bg-primary text-primary-foreground text-xs h-7">Save</Button>
                  <Button size="sm" variant="ghost" onClick={() => setEditingProfile(false)} className="text-xs h-7">Cancel</Button>
                </div>
              </div>
            ) : (
              <>
                <p className="font-semibold text-foreground text-sm truncate">{currentDisplayName}</p>
                <p className="text-xs text-muted-foreground truncate">{user?.email}</p>
                <button onClick={() => { setDisplayName(currentDisplayName); setEditingProfile(true); }} className="text-[11px] text-primary hover:underline mt-0.5">
        ...
        <div className="space-y-0.5">
          <button onClick={() => setShowShields(true)} className="w-full flex items-center gap-3 p-2.5 rounded-xl hover:bg-secondary/40 transition-colors text-foreground text-sm">
            <Shield className="h-4 w-4 text-primary" /> Shield Shop
          </button>
          <button onClick={() => setShowPowerUps(true)} className="w-full flex items-center gap-3 p-2.5 rounded-xl hover:bg-secondary/40 transition-colors text-foreground text-sm">
            <Zap className="h-4 w-4 text-primary" /> Power-Up Recovery
          </button>
          <button onClick={() => setShowImport(true)} className="w-full flex items-center gap-3 p-2.5 rounded-xl hover:bg-secondary/40 transition-colors text-foreground text-sm">
            <Upload className="h-4 w-4 text-primary" /> Import History
          </button>

          {/* Start Date */}
          <div className="w-full flex items-center justify-between p-2.5 rounded-xl text-foreground text-sm">
          ...
          <div className="w-full flex items-center justify-between p-2.5 rounded-xl text-foreground text-sm">
            <span className="flex items-center gap-3"><Bell className="h-4 w-4 text-primary" /> Notifications</span>
          ...
          <button onClick={handleResetDefaults} disabled={resetting} className="w-full flex items-center gap-3 p-2.5 rounded-xl hover:bg-destructive/10 transition-colors text-destructive text-sm">
            <RotateCcw className="h-4 w-4" /> {resetting ? "Resetting..." : "Reset All Data"}
          </button>

          <Button onClick={handleSignOut} variant="ghost" className="w-full justify-start text-destructive hover:text-destructive text-sm h-9">
